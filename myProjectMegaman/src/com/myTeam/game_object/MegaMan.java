...
public class MegaMan extends Character{
    ...
    public void Update() {
        ...
        if(isShooting){
            if(System.nanoTime() - lastShootingTime > 250*1000000){
                isShooting = false;
            }
        }
    ...
    public void jump() {
        ...
        // for clim wall
            ...
            if(getGameWorld().physicalMap.collisionWithRight(rectRightWall)!=null && getSpeedX() > 0){
                setSpeedY(-5.0f);
                //setSpeedX(-1);
                flyBackAnim.reset();
                flyForwardAnim.reset();
                //setDirection(LEFT_DIR);
            }else if(getGameWorld().physicalMap.collisionWithLeft(rectLeftWall)!=null && getSpeedX() < 0){
                setSpeedY(-5.0f);
                //setSpeedX(1);
                flyBackAnim.reset();
                flyForwardAnim.reset();
    ...
    public void attack() {
        ...
        if(!isShooting && !getIsDicking()){
            ...
            Bullet bullet = new BlueFire1(getPosX(), getPosY(), getGameWorld());
            if(getDirection() == LEFTDIR) {
                bullet.setSpeedX(-10);
